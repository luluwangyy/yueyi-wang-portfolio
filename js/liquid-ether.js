// Vanilla-JS port of the LiquidEther React component. The WebGL fluid
// simulation itself is framework-agnostic (plain Three.js classes) — only
// the React wrapper (useEffect/useRef/props) has been stripped out and
// replaced with a plain init function + returned control handle.
//
// Usage:
//   import { createLiquidEther } from "./liquid-ether.js";
//   const fx = createLiquidEther(containerEl, { colors: [...], ... });
//   fx.start(); fx.pause(); fx.dispose();

import * as THREE from "https://esm.sh/three@0.169.0";

export function createLiquidEther(container, opts = {}) {
  const options = {
    mouseForce: 20,
    cursorSize: 100,
    isViscous: false,
  ...
  class AutoDriver {
    constructor(mouse, manager, opts2) {
      this.mouse = mouse;
      this.manager = manager;
      this.enabled = opts2.enabled;
      this.speed = opts2.speed;
      this.resumeDelay = opts2.resumeDelay || 3000;
      this.rampDurationMs = (opts2.rampDuration || 0) * 1000;
      this.active = false;
...
`;
  ...
  class WebGLManager {
    ...
    /** Fully stop auto-resuming (used when the effect is scrolled away for good). */
    stop() {
      this._wantsRunning = false;
      this.pause();
    }
  ...
  // Positioning/overflow are handled by the .liquid-ether-root CSS class in
  // this project (absolute + inset:0 to fill .home-top), so we don't force
  // inline styles here — doing so would win over the stylesheet rule and
  // break the fill-parent layout.
  ...
  // logic the caller may layer on top via stop()/start().
